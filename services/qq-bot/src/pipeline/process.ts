...
export class ProcessStage extends PipelineStage {
  ...
  async execute(event: QQMessageEvent): Promise<StageResult> {
    ...
    // ─── 内置命令前置处理 ─────────────────────────────────────
    const cmdResult = this.handleCommand(event)
    if (cmdResult)
      return cmdResult
   ...
   */
  private handleCommand(event: QQMessageEvent): StageResult | null {
    const { prefix, enabled } = this.config.commands
    const text = event.text.trim()

    if (!text.startsWith(prefix))
      return null

    const [cmd, ..._args] = text.slice(prefix.length).split(COMMAND_SPLIT_RE)
    ...
    switch (command) {
      ...
      case 'clear':
        ...
        return {
          action: 'respond',
          payload: createTextResponse('已清空当前会话上下文 ✓', event.id),
        }
      default:
        return null
    }
